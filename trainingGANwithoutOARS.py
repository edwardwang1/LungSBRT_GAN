...
def getDLoss(g, d, real_dose, alt_condition, disc_alt_condition, adv_criterion):
    D_real = d(real_dose, disc_alt_condition)
    ...
    y_fake = g(alt_condition)
    D_fake = d(y_fake.detach(), disc_alt_condition)

    D_fake_loss = adv_criterion(D_fake, torch.zeros_like(D_fake))
    D_loss = (D_real_loss + D_fake_loss) / 2
    return D_loss, D_real_loss, D_fake_loss

def getGLoss(g, d, real_dose, alt_condition, disc_alt_condition, adv_criterion, voxel_criterion, alpha, beta, loss_type="l1"):
    y_fake = g(alt_condition)
    D_fake = d(y_fake, disc_alt_condition)
    ...
    return G_loss, G_Dcomp_loss_train, G_voxel_loss, G_loss, y_fake
...
def train(data_dir, patientList_dir, singleLesionList, save_dir, exp_name_base, exp_name, params):
    ...
    g.cuda()
    d = Discriminator(in_features=2, last_conv_kernalsize=disc_last_layer_ks)
    d.cuda()
    ...
    if pretrain_disc:
        for pretrain_epoch in range(pretrain_disc_epoch):
            ...
            for idx, volumes in enumerate(train_loader):
                ...
                D_real = d(real_dose, est_dose)
                D_fake = d(est_dose, est_dose)
    ...
    for epoch in epoch_loop:
        ...
        for idx, volumes in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast():
                D_loss, D_real_loss, D_fake_loss = getDLoss(g, d, real_dose,  alt_condition, prescription, adv_criterion)
            ...
            with torch.cuda.amp.autocast():
                ...
                G_loss, G_Dcomp_loss_train, voxel_loss_train, masked_voxel_loss_train, y_fake = getGLoss(g, d, real_dose,
        ...
        masked_train_mse_loss = L2_LOSS_sum(y_fake, real_dose)
        ...
        if epoch % log_interval == 0:
            ...
            for test_idx, test_volumes in enumerate(test_loader):
                with torch.no_grad():
                    ...
                    D_loss_test, D_real_loss_test, D_fake_loss_test = getDLoss(g, d, real_dose_test, alt_condition_test, est_dose_test, adv_criterion)
            ...
            # calculate extra losses and metrics
            y_fake_test = g(alt_condition_test).detach()
            test_mse_loss = L2_LOSS_sum(y_fake_test, real_dose_test) / torch.numel(y_fake_test)
            masked_test_mse_loss = L2_LOSS_sum(y_fake_test, real_dose_test)
    ...
    writer.add_hparams(
        {"epochs": num_epochs, "alpha": alpha, "beta": beta, "loss_type": loss_type, "d_update_ratio": d_update_ratio,
         ...
         "adv_loss_type": adv_loss_type, "pretrain_disc": pretrain_disc, "pretrain_disc_epoch": pretrain_disc_epoch, "disc_last_layer_ks": disc_last_layer_ks},
